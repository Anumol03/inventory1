...
class PurchaseItemForm(forms.ModelForm):
    class Meta:
        model = PurchaseItem
        fields = ['stock', 'quantity', 'perprice', 'discount']
    ...
    def __init__(self,
        ...
        self.fields['stock'].widget.attrs.update({
            'class': 'textinput form-control setprice stock',
            'required': 'true','id':'itemDropdown1','onchange':'displayPrice()1',
            'data-price':'stock.price','value':'stock',
        })
        self.fields['quantity'].widget.attrs.update({'class': 'textinput form-control setprice quantity', 'min': '0', 'required': 'true'})
        self.fields['perprice'].widget.attrs.update({'class': 'textinput form-control setprice price', 'min': '0', 'required': 'true','id':'price'})
        self.fields['discount'] = forms.IntegerField(
            label='Discount',
            widget=forms.NumberInput(attrs={'class': 'textinput form-control setprice discount', 'min': '0'}),
            initial=0
        )
    ...
    def clean(self):
        ...
        if stock:
            # Set perprice based on the selected stock's price
...
class SaleItemForm(forms.ModelForm):
    def __init__(self, *args, **kwargs):
        ...
        self.fields['discount'] = forms.IntegerField(
            label='Discount',
            widget=forms.NumberInput(attrs={'class': 'textinput form-control setprice discount', 'min': '0'}),
            initial=0  # You can set the default value for the discount here
        )
    class Meta:
        model = SaleItem
        fields = ['stock', 'quantity', 'perprice','discount']
